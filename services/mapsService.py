...
class MapsService:
    ...
    def completeGraphe(self, listCoordCity):
        for i in range(len(listCoordCity)):
            for j in range(i+1, len(listCoordCity)):
                folium.PolyLine([[listCoordCity[i].X, listCoordCity[i].Y], [listCoordCity[j].X, listCoordCity[j].Y]], tooltip="Coast").add_to(self.map)
